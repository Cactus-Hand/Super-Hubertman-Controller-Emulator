# Features
- Mouse Joystick: When enabled, the mouse will behave like a joystick. It will only move within a specific adjustable radius while M1 is held, and rubber band to the center otherwise
- Key-bound Clickers: Bind a specific mouse coordinate to a key on the keyboard. Whenever this key is pressed, the mouse will teleport to this coordinate and click before teleporting back.
